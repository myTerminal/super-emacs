...
| c     | Toggles showing of current buffer character count                |
| \/    | Toggles auto filling column as you type                          |
...
*** Preventing accidental quit

*super-emacs* cannot be quit using ~C-x C-c~ to prevent losing unsaved buffer layouts and other settings by accident. To quit Emacs, the following key-binding has been chosen such that it's least likely to get triggered unintentionally.
